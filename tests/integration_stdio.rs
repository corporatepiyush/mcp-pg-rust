...
fn assert_success(resp: &Value, id: u64) {
    assert!(
        ...
        "Response {id} missing result: {resp}"
    );
    assert!(
        resp.get("error").is_none() || resp["error"].is_null(),
        "Response {id} has error: {resp}"
    );
...
fn test_stdio_full_workflow() {
    ...
    send_request(
        ...
        "tools/call",
        Some(
            json!({"name":"create_table","arguments":{"table":"stdio_test","columns":["id SERIAL PRIMARY KEY","name TEXT","val INTEGER"]}}),
        ),
        4,
    );
    let resp = read_response(&mut *stdout);
    ...
    send_request(
        ...
        "tools/call",
        Some(
            json!({"name":"execute_insert","arguments":{"sql":"INSERT INTO stdio_test (name, val) VALUES ('hello', 42)"}}),
        ),
        5,
    );
    let resp = read_response(&mut *stdout);
    ...
    send_request(
        ...
        "tools/call",
        Some(
            json!({"name":"execute_query","arguments":{"sql":"SELECT * FROM stdio_test ORDER BY id"}}),
        ),
        6,
    );
    let resp = read_response(&mut *stdout);
    ...
    send_request(
        &mut *stdin,
        "tools/call",
        Some(json!({"name":"list_tables","arguments":{}})),
        7,
    );
    let resp = read_response(&mut *stdout);
    ...
    send_request(
        &mut *stdin,
        "tools/call",
        Some(json!({"name":"execute_query","arguments":{"sql":"SELECT * FROM nonexistent"}})),
        8,
    );
    let resp = read_response(&mut *stdout);
    assert_error(&resp, 8);
    ...
    send_request(
        ...
        "tools/call",
        Some(
            json!({"name":"execute_update","arguments":{"sql":"UPDATE stdio_test SET val = 99 WHERE name = 'hello'"}}),
        ),
        10,
    );
    let resp = read_response(&mut *stdout);
    ...
    send_request(
        ...
        "tools/call",
        Some(
            json!({"name":"execute_query","arguments":{"sql":"SELECT val FROM stdio_test WHERE name = 'hello'"}}),
        ),
        11,
    );
    let resp = read_response(&mut *stdout);
    ...
    send_request(
        ...
        "tools/call",
        Some(
            json!({"name":"execute_delete","arguments":{"sql":"DELETE FROM stdio_test WHERE name = 'hello'"}}),
        ),
        12,
    );
    let resp = read_response(&mut *stdout);
    ...
    send_request(
        &mut *stdin,
        "tools/call",
        Some(json!({"name":"drop_table","arguments":{"table":"stdio_test","if_exists":true}})),
        13,
    );
    let resp = read_response(&mut *stdout);
    ...
    send_request(
        &mut *stdin,
        "tools/call",
        Some(json!({"name":"show_current_user","arguments":{}})),
        14,
    );
    let resp = read_response(&mut *stdout);
