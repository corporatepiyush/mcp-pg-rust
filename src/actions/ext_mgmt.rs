...
pub async fn list_extensions(client: &Client, _params: &Option<&Value>) -> MCPResult<Value> {
    ...
    let extensions: Vec<Value> = rows.iter().map(|row| {
        json!({
...
pub async fn create_extension(client: &Client, params: &Option<&Value>) -> MCPResult<Value> {
    ...
    let version = params.as_ref().and_then(|p| p.get("version").and_then(|v| v.as_str()));
    let if_not_exists = params.as_ref().and_then(|p| p.get("if_not_exists").and_then(|v| v.as_bool())).unwrap_or(false);

    let mut sql = "CREATE EXTENSION".to_string();
    if if_not_exists { sql.push_str(" IF NOT EXISTS"); }
    sql.push_str(&format!(" {}", crate::validation::quote_ident(name)));
    if let Some(s) = schema { sql.push_str(&format!(" SCHEMA {}", crate::validation::quote_ident(s))); }
    if let Some(v) = version { sql.push_str(&format!(" VERSION '{}'", v.replace('\'', "''"))); }
