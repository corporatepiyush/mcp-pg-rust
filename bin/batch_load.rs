...
impl Generator {
    async fn gen_core(&self) -> Result<(Vec<i64>, Vec<i64>, Vec<i64>, Vec<i64>)> {
        ...
        let o_ids = self
            .ins(
                "organizations",
                &["name", "domain", "revenue", "employee_count", "country"],
                o_r,
                Some("id"),
            )
            .await?;
        ...
        for &oid in &o_ids {
            for _ in 0..20 {
                u_r.push(vec![
                    ...
                    json!(FirstName().fake::<String>()),
                    json!(LastName().fake::<String>()),
                    json!("user"),
                ]);
            }
        }
    ...
    async fn gen_projects(&self, o_ids: &[i64], u_ids: &[i64]) -> Result<(Vec<i64>, Vec<i64>)> {
        ...
        for &oid in o_ids {
            for _ in 0..5 {
                p_r.push(vec![
                    ...
                    json!(format!(
                        "Project {}",
                        Words(1..2).fake::<Vec<String>>().join(" ")
                    )),
                    json!("active"),
                    json!(2024),
                    json!(50000.0),
        ...
        let p_ids = self
            .ins(
                "projects",
                &["org_id", "name", "status", "start_year", "budget"],
                p_r,
                Some("id"),
            )
            .await?;
        ...
        self.ins(
            "project_members",
            &["project_id", "user_id", "role", "hours_allocated"],
            pm_r,
            None,
        )
        .await?;
        ...
        for &pid in &p_ids {
            for _ in 0..10 {
                t_r.push(vec![
                    json!(pid),
                    json!(u_ids[0]),
                    json!(Words(2..4).fake::<Vec<String>>().join(" ")),
                    json!("todo"),
                    json!("high"),
                    json!(202406),
                ]);
...
impl Generator {
    async fn gen_sales(&self, o_ids: &[i64]) -> Result<(Vec<i64>, Vec<i64>, Vec<i64>, Vec<i64>)> {
        ...
        let c_ids = self
            .ins(
                "clients",
                &["org_id", "name", "email", "company_name", "industry"],
                c_r,
                Some("id"),
            )
            .await?;
        ...
        let pr_ids = self
            .ins(
                "products",
                &["org_id", "name", "category", "price", "sku"],
                pr_r,
                Some("id"),
            )
            .await?;
...
impl Generator {
    async fn gen_misc(&self, o_ids: &[i64], u_ids: &[i64], e_ids: &[i64]) -> Result<()> {
        let dt_r = vec![
            vec![json!("Contract"), json!("Legal contracts")],
            vec![json!("Invoice"), json!("Customer invoices")],
        ];
        let dt_ids = self
            .ins("document_types", &["name", "description"], dt_r, Some("id"))
            .await?;
        ...
        self.ins(
            "employee_training",
            &["employee_id", "program_id", "completion_date"],
            et_r,
            None,
        )
        .await?;
...
async fn run() -> Result<()> {
    ...
    info!(
        "Comprehensive high-volume generation: target={}, batch={}",
        a.target_rows, a.batch_size
    );
    ...
    g.gen_remaining(
        &o_ids, &u_ids, &e_ids, &c_ids, &pr_ids, &ord_ids, &v_ids, &po_ids, &p_ids,
    )
    .await?;
    ...
    info!(
        "Base entities created: {} rows. Filling remaining {} rows with audit logs...",
        current,
        a.target_rows.saturating_sub(current)
    );
    ...
    while current < a.target_rows {
        ...
        if current + (au.len() as u64) >= a.target_rows {
            break;
        }
    }
    if !au.is_empty() {
    ...
    info!(
        "Generation complete: {} total rows in {:?}",
...
impl Generator {
    async fn gen_hr_extra(&self, u_ids: &[i64], e_ids: &[i64]) -> Result<()> {
        ...
        self.ins("feedback", &["user_id", "rating", "comment"], f_r, None)
            .await?;
        ...
        for &eid in e_ids {
            att_r.push(vec![
                json!(eid),
                json!("2024-06-01"),
                json!("2024-06-01 09:00:00"),
                json!("2024-06-01 17:00:00"),
                json!("present"),
            ]);
        }
        self.ins(
...
impl Generator {
    ...
    async fn gen_admin_depth(&self, o_ids: &[i64], u_ids: &[i64]) -> Result<()> {
        ...
        self.ins("api_keys", &["org_id", "user_id", "api_key"], ak_r, None)
            .await?;
        ...
        self.ins(
            "report_schedules",
            &["report_id", "frequency", "recipients"],
            rs_r,
            None,
        )
        .await?;
    ...
    async fn gen_remaining(
        ...
        let ct_r = vec![
            vec![json!("Service")],
            vec![json!("Maintenance")],
            vec![json!("Lease")],
        ];
        let ct_ids = self
            .ins("contract_types", &["name"], ct_r, Some("id"))
            .await?;
        ...
        self.ins(
            ...
            &["equipment_id", "user_id", "assigned_date"],
            ea_r,
            None,
        )
        .await?;
        ...
        let hol_r = vec![
            vec![json!("New Year"), json!("2024-01-01"), json!("US")],
            vec![json!("Christmas"), json!("2024-12-25"), json!("US")],
        ];
        self.ins(
            "holidays",
        ...
        let et_r = vec![
            vec![
                json!("Welcome Email"),
                json!("Welcome {{name}}"),
                json!("Hello {{name}}, welcome!"),
            ],
            vec![
                json!("Invoice Reminder"),
                json!("Invoice Due"),
                json!("Your invoice is due."),
            ],
        ];
        self.ins("email_templates", &["name", "subject", "body"], et_r, None)
            .await?;
